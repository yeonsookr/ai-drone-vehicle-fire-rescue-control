...
<template>
  <div class="flex flex-col gap-4 min-h-0">
    <div class="flex-1 bg-gray-800/90 backdrop-blur rounded-lg border border-gray-700 p-2 flex flex-col">
      <div class="text-xs text-gray-400 mb-2 px-1">Camera Feed</div>
      <div class="flex-1 grid grid-cols-4 grid-rows-4 gap-1.5 min-h-0">
        <div v-for="i in 16" :key="i" class="bg-gray-900/80 rounded border border-gray-700 flex items-center justify-center text-xs text-gray-600 relative overflow-hidden" :class="{ 'ring-1 ring-gray-400': i === 1 }">
    ...
    <div class="h-14 bg-gray-800/90 backdrop-blur rounded-lg border border-gray-700 flex items-center px-4 shrink-0">
      <span class="text-xs text-gray-500">Drones: {{ activeDrones }} | Vehicles: {{ activeVehicles }} | Streaming: {{ streaming }}</span>
    </div>
  </div>
</template>
