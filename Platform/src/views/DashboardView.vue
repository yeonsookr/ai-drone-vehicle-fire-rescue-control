...
function goToCurrentLocation() {
  ...
  navigator.geolocation.getCurrentPosition(
    (pos) => {
      ...
      kakaoMap!.setCenter(latlng); kakaoMap!.setLevel(5); currentLevel.value = 5
    }, () => {},
...
<template>
  <div class="flex-1 flex flex-col min-h-0">
    <header class="h-14 flex items-center justify-between px-6 border-b border-gray-800 shrink-0">
      ...
      <span class="text-xs px-2.5 py-1 rounded-full" :class="telemetry.connected ? 'bg-green-900 text-green-300' : 'bg-red-900 text-red-300'">{{ telemetry.connected ? '● Connected' : '○ Disconnected' }}</span>
    </header>
    <section class="h-16 flex items-center gap-4 px-6 border-b border-gray-800 shrink-0">
      <div class="flex items-center gap-3 flex-1 h-10 bg-gray-800 rounded-lg px-4">
        ...
        <div><div class="text-[11px] text-gray-500 leading-tight">Active Drones</div><div class="text-base font-bold text-gray-100 leading-tight">{{ activeDrones }}</div></div>
      </div>
      <div class="flex items-center gap-3 flex-1 h-10 bg-gray-800 rounded-lg px-4">
        <Satellite class="w-5 h-5 text-gray-400 shrink-0" />
        <div><div class="text-[11px] text-gray-500 leading-tight">Gateways</div><div class="text-base font-bold text-gray-100 leading-tight">{{ activeGateways }}</div></div>
      </div>
      <div class="flex items-center gap-3 flex-1 h-10 bg-gray-800 rounded-lg px-4">
        <Camera class="w-5 h-5 text-gray-400 shrink-0" />
        <div><div class="text-[11px] text-gray-500 leading-tight">Streaming</div><div class="text-base font-bold text-gray-100 leading-tight">{{ streaming }}</div></div>
      </div>
      <div class="flex items-center gap-3 flex-1 h-10 bg-gray-800 rounded-lg px-4">
        <TriangleAlert class="w-5 h-5 text-gray-400 shrink-0" />
        <div><div class="text-[11px] text-gray-500 leading-tight">Alerts</div><div class="text-base font-bold text-gray-100 leading-tight">0</div></div>
      </div>
    </section>
    <section class="flex-1 flex gap-4 p-4 min-h-0">
      <div ref="mapContainer" class="flex-1 bg-gray-800 rounded-lg border border-gray-700 relative overflow-hidden">
        ...
        <div v-if="mapReady" class="absolute top-3 right-3 z-10 flex flex-col gap-1.5">
          ...
          <button @click="goToCurrentLocation" title="My location" class="w-9 h-9 bg-gray-900/80 hover:bg-gray-900 rounded-lg border border-gray-600 flex items-center justify-center text-xs text-gray-300 transition-colors"><Crosshair class="w-4 h-4" /></button>
        </div>
        <div v-if="mapReady" class="absolute bottom-3 right-3 z-10 flex flex-col gap-px rounded-lg overflow-hidden shadow-lg">
          <button @click="zoomIn" title="Zoom in" class="w-9 h-9 bg-gray-900/90 hover:bg-gray-700 flex items-center justify-center text-sm text-gray-200 transition-colors border-b border-gray-700"><Plus class="w-4 h-4" /></button>
          <button @click="zoomOut" title="Zoom out" class="w-9 h-9 bg-gray-900/90 hover:bg-gray-700 flex items-center justify-center text-sm text-gray-200 transition-colors"><Minus class="w-4 h-4" /></button>
        </div>
      </div>
      <div class="flex-1 flex flex-col gap-4 min-h-0">
        ...
        <div class="h-14 bg-gray-800 rounded-lg border border-gray-700 flex items-center px-4 shrink-0">
          <span class="text-xs text-gray-500">Drones: {{ activeDrones }} | Gateways: {{ activeGateways }} | Recording: {{ streaming }}</span>
        </div>
      </div>
    </section>
  </div>
